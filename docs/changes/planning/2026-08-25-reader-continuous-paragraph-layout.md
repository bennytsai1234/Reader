...
讓 Hybrid 閱讀器把效能 block 視為載入／排程單位，而不是排版段落；同一個邏輯段落無論以何種 block 邊界切割，最終的換行、總高度、字元幾何與錨點位置都必須等價於連續排版。
...
ReaderV2ContentTransformer
  ...
  -> ChapterBlock
  -> LayoutTask
  -> 一個 ui.Paragraph / block
  -> 一個 Sliver item / block
  -> DocumentIndex 的 block 高度前綴和
```
...
快取與視口還有一個必要連帶：`StyleFingerprint` 與 `MetricsDiskCache` 目前以 style／viewport／章節 content hash／`BlockKey` 識別 metrics，但沒有記錄 block 的實際 range 或 segmentation。章節被 repository 淘汰時，`HybridReaderScreen._onChapterEvent` 的 `evicted` 分支只移除 `_blocks`，沒有像 `invalidated` 分支一樣清理 metrics／Paragraph／DocumentIndex。修正後必須確保 block 仍可作為載入單位，但任何 Paragraph、metrics、char-range 或高度都不會把人工切割重新當成視覺語義；若實作需要新增 layout identity 或 namespace 維度，需同步所有相關快取與失效路徑。
...
- block 仍可作為 lazy loading、pump 預算、admission 與 LRU 的工作單位；不得以一次建立整本書或無界章節 Paragraph 來繞過問題，也不得在拖曳期間違反現有 I4 契約直接 layout。
