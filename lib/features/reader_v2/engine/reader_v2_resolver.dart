...
import 'reader_v2_chapter_repository.dart';
import 'reader_v2_chapter_view.dart';
import 'reader_v2_layout_engine.dart';
...
class ReaderV2Resolver {
  ...
  Future<TextPage> pageForLocation(ReaderV2Location location) async {
    final layout = await ensureLayout(location.chapterIndex);
    return layout.pageForCharOffset(location.charOffset);
  }

  Future<TextPage?> nextPage(TextPage page, {bool allowAsyncLoad = false}) async {
    final layout =
        allowAsyncLoad
  ...
  Future<TextPage?> prevPage(TextPage page, {bool allowAsyncLoad = false}) async {
    final layout =
        allowAsyncLoad
            ? await ensureLayout(page.chapterIndex)
            : cachedLayout(page.chapterIndex);
  ...
  ReaderV2PageAddress addressOf(TextPage page) {
